// src/app/api/sms-optout/route.ts
// POST /api/sms-optout — records opt-out via SMS (no token needed, uses CWID directly)
// Called by the phone mockup when student texts N.
...
export async function POST(req: NextRequest) {
  ...
  for (const prog of (programs || ["eops"])) {
    ...
    await addOutreachEntry({
      ...
      action: "opted_out",
      timestamp: now,
      details: `Student opted out via SMS reply (N)`,
      staff_name: null,
    });
  }
